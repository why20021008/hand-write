...
class mainwindow(QMainWindow, Ui_Form):
    ...
    def __init__(self):
        ...
        self.pushButton.clicked.connect(
            lambda: self.lineEdit.setText(getfile()))
        self.pushButton_2.clicked.connect(
            lambda: self.lineEdit_2.setText(getfile()))
    ...
    def baocun(self):
        ...
        if file_path != "":
            file_text = "%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s" % (
                ...
                ), self.lineEdit_12.text(), self.lineEdit.text(),
                self.lineEdit_2.text(), self.lineEdit_7.text(
                ), self.spinBox.text(), self.lineEdit_8.text(),
                self.spinBox_2.text(), self.lineEdit_9.text(
                ), self.spinBox_3.text(), self.spinBox_5.text(),
                self.spinBox_4.text(), self.doubleSpinBox_6.text(
    ...
    def yulan(self):
        try:
            ...
            logging.error("preview error {}".format(e))
            QMessageBox.information(self, "预览错误", "预览错误")
    ...
    def daochu(self):
        try:
            ...
            if ziti == "" or zspjj == "" or bianju_down == "":
                ...
                def run():
                    ...
                    for i, im in enumerate(images):
                        ...
                        im.save(
                            "./output/{}.png".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                    self.sendmsg.emit()
            ...
            logging.error("export error {}".format(e))
            QMessageBox.information(self, "预览错误", "预览错误")
